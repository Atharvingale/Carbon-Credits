-- FIXED DATABASE SCRIPT - Corrected column names to match your schema
...
SELECT 
    ...
    p.id,
    p.name,  -- Using 'name' column from projects table
    p.description,
    p.location,
    p.ecosystem_type,
    p.area,
...
-- ==================================================
-- 5. SAFELY HANDLE TRIGGERS AND FUNCTIONS
-- ==================================================

-- Drop only the specific triggers we want to recreate
...
-- Drop only the project-specific function (not the shared one)
DROP FUNCTION IF EXISTS create_project_from_submission();

-- Function to create project from approved submission (CORRECTED COLUMN NAMES)
...
BEGIN
    ...
    IF NEW.status = 'approved' AND (OLD.status IS NULL OR OLD.status != 'approved') THEN
        -- Insert into projects table with CORRECT column names
        INSERT INTO projects (
            name,                    -- Using 'name' not 'project_name'
            description,
            project_type,
            location,
            country,
            region,
            area,                    -- Using 'area' 
            ecosystem_type,          -- This column exists in your schema
            estimated_credits,
            submitted_by,
            organization_name,
            contact_email,
            ...
            approved_by,
            approved_at,
            carbon_data,             -- This column exists in your schema
            calculated_credits,      -- This column exists in your schema
            calculation_data,        -- This column exists in your schema
            calculation_timestamp    -- This column exists in your schema
        ) VALUES (
            NEW.title,               -- submission.title → projects.name
            NEW.description,
            COALESCE(NEW.ecosystem_type, 'blue_carbon'),
            NEW.location,
            'Unknown',
            'Unknown', 
            NEW.project_area,        -- submission.project_area → projects.area
            NEW.ecosystem_type,
...
-- Create triggers (reusing the existing shared function)
...
-- Reuse the existing update_updated_at_column() function for our triggers
...
-- Show recent submissions with correct column names
SELECT 
...
-- Show recent projects with correct column names
SELECT 
    'Recent Projects:' as info,
    id, 
    name,                    -- Using correct 'name' column
    organization_name, 
    status, 
    created_at::date,
    submitted_by_user
...
-- Success message
SELECT 'Database fix completed successfully with correct column names!' AS status;
